...
from draugr.torch_utilities.optimisation.parameters.trainable import trainable_parameters

from neodroidaudition.classification.procs import single_epoch_evaluation, single_epoch_fitting
...
from draugr.torch_utilities import TensorBoardPytorchWriter, get_model_hash, get_num_parameters, global_torch_device, load_model, save_model
...
def training_procedure(n_epoch=60, train: bool = True, load_previous: bool = True):
  ...
  if load_previous:
    candidate = load_model(model_name=persistence_model_name, model_directory=model_storage_path, raise_on_failure=False)
    if candidate:
      model = candidate
  ...
  print(f"Number of trainable parameters: {get_num_parameters(model, only_trainable=True)}")
  ...
  if train:
    ...
    with TensorBoardPytorchWriter(PROJECT_APP_PATH.user_log / model_name / str(time.time())) as writer:
      best_valid_acc = math.inf
      for epoch in tqdm(range(1, n_epoch + 1), total=n_epoch, desc='Epoch #'):
        single_epoch_fitting(model, optimiser,
                             train_loader,
                             epoch=epoch,
                             writer=writer,
                             device_=device)
        ...
        if acc < best_valid_acc:
          best_valid_acc = acc
          save_model(model, model_name=persistence_model_name, save_directory=model_storage_path)
