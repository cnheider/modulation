...
if __name__ == '__main__':
  def asiuhda():
    ...
    d = 1 / 10
    t = range(100)
    a = SignalGenerator(d, sin, cos, lambda x:cos(x + 1), lambda x:sin(x + 2), lambda x:cos(x + 3), lambda x:sin(x + 4), square)
...
'''
  ...
  def pygame_ex2():
    ...
    arr = numpy.array([4096 * numpy.sin(2.0 * numpy.pi * freq * x / sample_rate) for x in range(0, sample_rate)]).astype(numpy.int16)
