...
def additive_white_noise(signal, signal_noise_ratio, sampling_rate, export_path=None) -> Iterable:
  noise = numpy.random.normal(0, math.sqrt(root_mean_square(signal) ** 2 / (pow(10, signal_noise_ratio / 10))), signal.shape[-1])
  if export_path:
...
if __name__ == '__main__':
  ...
  def asda():
    ...
    additive_white_noise(files[0], 10, sr[0], export_path=Path('exclude') / 'white_gaussian_noise.wav')
