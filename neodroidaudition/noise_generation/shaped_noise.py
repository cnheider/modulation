...
from neodroidaudition.audio_utilities.funcs import hilbert_envelope
...
def generate_speech_shaped_noise(samples: Iterable[numpy.ndarray], sampling_rate, *, export_path: Path = None, long_term_avg=True,window_length_sec=20/1000,p_overlap=0.5) -> Iterable:
  '''
  ...
  noise = spectrum_like_noise(samples, sampling_rate=sampling_rate, long_term_avg=long_term_avg,window_length_sec=window_length_sec,p_overlap=p_overlap)
  if export_path:
...
if __name__ == '__main__':
  def reald():
    ...
    files, sr = zip(*[(v[0].numpy(), v[1]) for _, v in zip(range(20), libri_speech)])
    assert all([sr[0] == s for s in sr[1:]])
    sr = sr[0]

    # from neodroidaudition.noise_generation import generate_babble_noise
    # files = generate_babble_noise(samples, sampling_rate)
    samples_len = len(numpy.concatenate(files, -1)[0])

    noise_welch = generate_speech_shaped_noise(files, sr, long_term_avg=True, export_path=Path('exclude') / 'ssn_welch.wav')[0]
    noise_fft = generate_speech_shaped_noise(files, sr, long_term_avg=False, export_path=Path('exclude') / 'ssn_fft.wav')[0]
  ...
  def synth():
    ...
    t = numpy.linspace(0, signal_length_sec, int(1000 * signal_length_sec), endpoint=False)
    signal = numpy.sin(50 * 2 * numpy.pi * t) + numpy.sin(200 * 2 * numpy.pi * t) + numpy.sin(400 * 2 * numpy.pi * t)
    sr = len(t) / signal_length_sec

    noise_welch = spectrum_like_noise(signal, sampling_rate=sr, long_term_avg=True)[0]
    noise_welch_full = spectrum_like_noise(signal, sampling_rate=sr, long_term_avg=True, window_length_sec=signal_length_sec,p_overlap=0)[0]
    noise_fft = spectrum_like_noise(signal, sampling_rate=sr, long_term_avg=False)[0]
