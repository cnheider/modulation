...
def batch_pad(image_batch: Sequence, mask_batch: Sequence) -> Tuple[Sequence, Sequence]:
  ...
  mask_batch = [
      ...
      functional.pad(mask, [0, max_width - mask.size(1), 0, max_height - mask.size(0)])
...
def pad_sequence(batch: Iterable[torch.Tensor]) -> torch.Tensor:
  ...
  batch = torch.nn.utils.rnn.pad_sequence(batch, batch_first=True, padding_value=0.)  # Make all tensor in a batch the same length by padding with zeros
  return batch.permute(0, 2, 1)


def min_length_pad(sequence: torch.Tensor, min_length: int, padding_value: Number = 0) -> torch.Tensor:
  '''
  ...
  if length < min_length:
    ...
    out_tensor = sequence.new_full(out_dims, padding_value)  # .data.new(*out_dims).fill_(padding_value)
