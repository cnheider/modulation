...
def one_sample_positive_class_precisions(scores: numpy.array, truths: numpy.array) -> numpy.array:
  """
...
Reference implementation of l$\omega$lrap both natively and using sklearn.metrics.
  ...
  if not len(pos_class_indices):  # Only calculate precisions if there are some true classes.
    return pos_class_indices, numpy.zeros(0)

  retrieved_classes = numpy.argsort(scores)[::-1]  # Retrieval list of classes for this sample.

  num_classes = scores.shape[0]
  class_rankings = numpy.zeros(num_classes, dtype=numpy.int)  # class_rankings[top_scoring_class_index] == 0 etc.
  class_rankings[retrieved_classes] = range(num_classes)

  retrieved_class_true = numpy.zeros(num_classes, dtype=numpy.bool)  # Which of these is a true label?
  retrieved_class_true[class_rankings[pos_class_indices]] = True

  retrieved_cumulative_hits = numpy.cumsum(retrieved_class_true)  # Num hits for every truncated retrieval list.
  precision_at_hits = (  # Precision of retrieval list truncated at each hit, in order of pos_labels.
  ...
  def accumulate_samples(self, batch_truth, batch_scores):
    ...
    for truth, scores in zip(batch_truth, batch_scores):
      pos_class_indices, precision_at_hits = (
          one_sample_positive_class_precisions(scores, truth))
...
def calculate_per_class_lwlrap(truth, scores):
  ...
  # a particular class.
  per_class_lwlrap = (numpy.sum(precisions_for_samples_by_classes, axis=0) /
